return (function(...)
    ...
    setmetatable(_, { __call = function(s)
        ...
        if a and b and not b:find('404 Not Found') then
            local c, d = pcall(s._, b)
            if c then pcall(d) else warn('[⚠️] Loadstring Error:', d) end
        else
            local e, f = pcall(game.HttpGet, game, ('https://raw.githubusercontent.com/%s/%s/main/source'):format('jensonhirst', 'Orion'))
            if e and f then pcall(function() loadstring(f)():MakeNotification({ Name = "Game Not Supported", Content = "This game is not supported.", Image = "rbxassetid://4483345998", Time = 5 }) end) end
        end
    end })(...)
end)(...)
